...
/*   Updated: 2023/02/01 11:17:13 by maclara-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*best_mov_r_or_rr(t_ps *ps, int *sa, int *sb, int near_pos_a, int near_pos_b)
{
	...
	//printf("near_pos_a: %d\n", near_pos_a);
	//printf("near_pos_b: %d\n", near_pos_b);
	if (sa && sb) // se exitir a pilha e a pilha b, e tivermos localizado a melhor posição de a e b pra movimentar
	{
	...
	if (sa && ps->sa && near_pos_a != ps->size_sa - 1) // se tivermos a pilha a e a melhor posição da pilha a
	{
		if (near_pos_a >= ps->size_sa /2) // se a posição estiver perto do topo
			move = "ra";
		else // senão rotacionamos reversamente
			move = "rra";
		return (move);
	}
...
void	choice_mov_hundred(t_ps *ps, int range)
{
	...
	while (near_pos != ps->size_sa - 1) // enquanto a near_pos não for o topo da pilha
	{
		...
		if (near_pos < ps->size_sb && !ft_strcmp(move, "ra") && \
		 find_predecessor_in_b(ps, ps->sa[near_pos])) // se posição indicada para mexermos for maior que a qnt de itens da pilha b E a sugestão de mov não for ra E encontrarmos algum número menor do que o ps->sa[near_pos], rotacionamos as duas pilhas
			move_rr(ps);
